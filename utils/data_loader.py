...
class DataLoaderWrapper(L.LightningDataModule):
  ...
  def train_dataloader(self):
    # ... (code hiện tại)
    samples_weight = torch.tensor([weight[label.item()] for label in labels])  # Đảm bảo weight từ imbalance
    sampler = WeightedRandomSampler(weights=samples_weight, num_samples=len(samples_weight), replacement=True)
    return DataLoader(self.dataset, batch_size=self.train_batch_size, sampler=sampler, num_workers=self.num_workers)
...
class DataLoaderWrapper(L.LightningDataModule):
  ...
  def train_dataloader(self):
    ...
    samples_weight = torch.tensor([weight[label.item()] if isinstance(label, torch.Tensor) else weight[label] for label in labels])
